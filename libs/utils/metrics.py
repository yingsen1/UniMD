...
def load_gt_seg_from_json(json_file, split=None, label='label_id', label_offset=0):
    ...
    for k, v in json_db.items():
        ...
        if split is not None and v['subset'].lower() != split.lower():
            continue
...
class ANETdetection(object):
    ...
    def evaluate(self, preds, verbose=True):
        ...
        if verbose:
            """
            |tIoU = 0.30: mAP = 27.05 (%) Recall@1x = 72.08 (%) Recall@5x = 85.80 (%)
            |tIoU = 0.40: mAP = 25.00 (%) Recall@1x = 66.26 (%) Recall@5x = 82.22 (%)
            |tIoU = 0.50: mAP = 22.15 (%) Recall@1x = 59.22 (%) Recall@5x = 77.14 (%)
            |tIoU = 0.60: mAP = 18.46 (%) Recall@1x = 51.27 (%) Recall@5x = 69.40 (%)
            |tIoU = 0.70: mAP = 14.08 (%) Recall@1x = 42.12 (%) Recall@5x = 57.59 (%)
            """
            # print the results

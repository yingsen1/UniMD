...
code from :https://github.com/EGO4D/episodic-memory/blob/main/NLQ/VSLNet/README.md
"""
...
def convert_result_to_ego4d(results, mr_format_result):
    ...
    for one_vid in results:
        ...
        for lbl in sorted_res[vid].keys():
            ...
            mr_format_result["%s_%d" % (vid, lbl)]["predicted_times"] = pred_period
    return mr_format_result
...
def evaluate_nlq_performance(
    ...
    for pred_datum in predictions:
        ...
        if len(pred_datum["predicted_times"]) > 0:
            overlap = compute_IoU(
